...
public class CityLODCulling : MonoBehaviour
{
    ...
    private void Awake()
    {
        _r   = GetComponentsInChildren<Renderer>(true);
        _d2  = CullingDistance * CullingDistance;
        _off = UnityEngine.Random.Range(0, 8);
    }

    private void Update()
    {
        if ((Time.frameCount + _off) % 8 != 0) return;
        var cam = Camera.main;
        if (cam == null || _r == null) return;
        bool vis = (transform.position - cam.transform.position).sqrMagnitude < _d2;
        foreach (var r in _r) if (r != null && r.enabled != vis) r.enabled = vis;
    }
}